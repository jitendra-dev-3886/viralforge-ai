...
export default function BillingPage() {
    ...
    return <div className="mx-auto max-w-6xl space-y-6 pb-10">
        ...
        {data?.limits_exempt ? <p className="rounded-xl bg-emerald-50 p-4 text-sm text-emerald-900">Your account has unlimited brands and exports with no plan expiry. No upgrade is required. AI provider charges still apply separately.</p> : <p className="rounded-xl bg-amber-50 p-4 text-sm text-amber-900">Payments are not connected yet. Prices below are proposed monthly prices. No payment is collected and no plan renews automatically. Contact the administrator for complimentary pilot access.</p>}
        ...
        {data && <section className="space-y-4"><div><h2 className="text-xl font-semibold">{plan?.name || "Unrecognized plan"} · {data.status}</h2><p className="mt-1 text-sm text-slate-500">{data.limits_exempt ? "No expiry. Usage below is tracked for reference." : <>Period ends {data.expires_at ? new Date(data.expires_at).toLocaleString() : "—"}. Existing saved content remains available after expiry.</>}</p></div>
            <div className="grid gap-3 sm:grid-cols-3">{meter("Brands", data.brands_used, plan ? plan.brands : 0)}{plan?.video_seconds == null ? meter("Video exports", usage.video_exports, plan?.video_exports) : meter("Video minutes", Math.round(usage.video_seconds / 60 * 100) / 100, (plan?.video_seconds || 0) / 60)}{meter("Image exports", usage.image_exports, plan ? plan.image_exports : 0)}</div>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{plans.map((item) => <article key={item.code} className={`rounded-2xl border bg-white p-5 ${item.code === plan?.code ? "border-indigo-500" : "border-slate-200"}`}>
            ...
            <ul className="space-y-2 text-sm text-slate-600"><li>{item.brands} brand{item.brands === 1 ? "" : "s"}</li><li>{item.video_exports != null ? `${item.video_exports} video exports` : item.video_seconds != null ? `${item.video_seconds / 60} video minutes` : "Unlimited video exports"}</li><li>{item.image_exports} image exports</li><li>Your own AI API key</li></ul>
