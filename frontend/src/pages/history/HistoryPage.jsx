...
export default function HistoryPage() {
  ...
  return (
    <div className="p-1 sm:p-4 lg:p-8">
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-500">
            Review every database-saved generation and its media.
          </p>
        </div>
        <Link
          to="/ai-studio"
      ...
      {loading && (
        <div className="flex items-center justify-center gap-3 rounded-3xl bg-white p-6 text-slate-500 sm:p-12">
          <LoaderCircle className="animate-spin" /> Loading history…
        </div>
      )}
      {!loading && !error && contents.length === 0 && (
        <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-6 text-center sm:p-12">
          ...
          <h2 className="mt-4 text-2xl font-semibold">No matching history</h2>
          <p className="mt-2 text-slate-500">
            Generate content or change the filters.
          </p>
        </div>
      )}
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setExpandedId(expanded ? null : content.id)}
                    className="flex flex-1 items-center justify-between gap-4 p-5 text-left"
                  >
                    <div>
                      ...
                      <p className="mt-2 text-sm text-slate-500">
                        ...
                        {(content.scenes || []).length} scene
                        {content.scenes?.length === 1 ? "" : "s"}
                      </p>
                    </div>
                  ...
                  <div className="mr-4 flex flex-wrap items-center gap-2">
                    ...
                    <DeleteButton
                      label={content.title || `content ${content.id}`}
                      description="This deletes the content and its associated scenes, voice records, and image records."
                      onDelete={() => deleteContent(content.id)}
                      onDeleted={() => {
                        setContents((items) =>
                          items.filter((item) => item.id !== content.id),
                        );
                        setExpandedId((id) => (id === content.id ? null : id));
                      }}
                    />
                  </div>
                </div>
                {expanded && (
                  <div className="border-t border-slate-200 bg-slate-50 p-4">
                    <PreviewPanel
                      ...
                      username={content.branding?.username}
      ...
      {!loading && contents.length > pageSize && (
