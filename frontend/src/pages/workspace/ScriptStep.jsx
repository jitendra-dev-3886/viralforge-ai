export default function ScriptStep({ content, onChange, busy }) {
    ...
    return <fieldset disabled={busy} className="space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">{[["title", "Title", 1], ["hook", "Opening hook", 2], ["script", "Full script", 4], ["caption", "Post caption", 4], ["hashtags", "Hashtags", 2], ["cta", "Call to action", 2]].map(([name, label, rows]) => <label key={name} className="text-sm font-medium">{label}<textarea className={field} rows={rows} value={Array.isArray(content[name]) ? content[name].join(", ") : content[name] || ""} onChange={(e) => onChange({ ...content, [name]: e.target.value })} /></label>)}</div>
