...
export default function MediaStep({ content, media, busy, run, refresh }) {
    return <div className="space-y-5">
        <div><h2 className="text-xl font-bold">Choose scene media</h2><p className="mt-1 text-sm text-slate-500">Keep the generated media, find a new stock asset, or use one already in this project.</p></div>
        <div className="grid gap-5 md:grid-cols-2">{content.scenes.map((scene, index) => <SceneMedia key={`${content.id}-${scene.id}`} scene={scene} index={index} media={media} busy={busy} run={run} refresh={refresh} />)}</div>
    </div>;
}

function SceneMedia({ scene, index, media, busy, run, refresh }) {
