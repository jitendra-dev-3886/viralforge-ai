...
import {
  generateProjectRender,
  uploadProjectMusic,
} from "../../api/projectRender";
...
export default function ContentFinisher({
  content,
  brandName: projectBrandName = "",
  logo = "",
  projectId,
  onBusyChange,
  ...
  const isImageContent =
    ...
    content.scenes.every((item) => item.media_type === "image");
  const mediaUrl = assetUrl(scene?.media_url);
  const resolvedLogo = assetUrl(
    logo || content.branding?.logo || content.generation_config?.branding?.logo,
  );
  const [brandName, setBrandName] = useState(
    content.generation_config?.branding?.brand_name ||
  ...
  const aspectRatio = useMemo(() => {
    ...
    if (type.includes("carousel") && platform.includes("instagram"))
      return "4 / 5";
  ...
  const saveLayout = async (audioOverride) => {
    if (!contentId)
      throw new Error("Content must be saved before editing its layout.");
    await updateContent(contentId, {
      generation_config: {
        ...(content.generation_config || {}),
        audio: audioOverride || {
          ...savedAudio.current,
          license_note: musicCredit,
        },
        ...
        branding: {
          ...
          logo:
            ...
            content.generation_config?.branding?.logo ||
            "",
        },
  ...
  const handleRender = async () => {
    ...
    try {
      ...
      if (!disableBackgroundMusic && music) {
        const uploaded = await uploadProjectMusic(
          resolvedProjectId,
          music,
          true,
        );
      ...
      const url =
        result.file_url || result.output_url || result.render?.file_url;
      const freshUrl = assetUrl(url);
      setRenderUrl(
        freshUrl
          ? `${freshUrl}${freshUrl.includes("?") ? "&" : "?"}v=${Date.now()}`
          : "",
      );
      setMessage(
        "Final video created with your saved visual and audio settings.",
      );
    } catch (error) {
      setMessage(
        error.response?.data?.detail ||
          error.message ||
          "Unable to create the final render.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const handleImageExport = async () => {
    ...
    try {
      if (content.scenes.some((item) => !item.id || !item.media_url))
        throw new Error(
          "Every slide needs saved media before exporting. Regenerate missing media first.",
        );
      await saveLayout();
      const response = await api.post(
        "/render/images",
        { scene_ids: content.scenes.map((item) => item.id) },
  ...
  const overlay = (name, children, className = "") => (
    <button
      ...
      style={{
        left: `${layout[name]?.x ?? defaultLayout[name].x}%`,
        top: `${layout[name]?.y ?? defaultLayout[name].y}%`,
        transform: "translate(-50%, -50%)",
        ...(preset
          ? {
              width:
                name === "logo"
                  ? `min(${preset.logo.size * 100}cqw, ${preset.logo.size * 100}cqh)`
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-bold text-slate-900">
            Finish & export{preset ? ` · ${preset.name}` : ""}
          </h3>
          <p className="text-sm text-slate-600">
            Drag the text, logo, and brand name. The layout applies to every
            scene in your final export.
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(240px,380px)_1fr]">
        ...
        >
          ...
          /\.(mp4|webm|mov)(\?|$)/i.test(mediaUrl || "") ? (
            ...
            <img
              src={mediaUrl}
              alt={scene.title || content.title || "Scene preview"}
              style={mediaPlacement(preset)}
            />
          )}
          {preset ? (
            ...
            <div className="absolute inset-x-0 bottom-0 h-[44%] bg-black/50" />
          )}
          {preset?.logo.visible !== false &&
            resolvedLogo &&
            overlay(
              "logo",
              preset ? (
                ...
                <img
                  src={resolvedLogo}
                  alt="Brand logo"
                  className="h-12 w-12 rounded-xl bg-white/90 object-contain p-1 shadow-lg sm:h-16 sm:w-16"
                />
              ),
            )}
          {overlay(
            ...
            preset ? (
              <StyledCaption
                preset={preset}
                text={scene.text || content.hook || content.title}
                opacity={opacity.text}
              />
            ) : (
              <span className="inline-block whitespace-pre-wrap rounded-lg bg-black/50 px-3 py-2 text-center text-base font-bold leading-relaxed text-white">
                {scene.text || content.hook || content.title}
              </span>
            ),
          )}
          {preset?.username.visible !== false &&
            resolvedUsername &&
            overlay(
              "username",
              <PlatformUsername
        ...
        <div className="space-y-4">
          <label className="block text-sm font-semibold text-slate-800">
            ...
            <select
              ...
              onChange={(event) => {
                ...
                setLayout(getDefaultLayout(getVisualStyle(event.target.value)));
                setRenderUrl("");
                setMessage("Design changed. Export again to apply it.");
              }}
              className="mt-2 block w-full rounded-xl border border-slate-200 bg-white p-3"
            >
            ...
            <span className="mt-2 block text-xs font-normal text-slate-500">
              Changing the design applies its logo, brand name and text
              positions. You can then drag to adjust.
            </span>
          </label>
          <label className="block text-sm font-semibold text-slate-800">
            Brand name
            ...
            <span className="mt-1 block text-xs font-normal text-slate-500">
              Use your full brand name. Save layout to apply it to exports.
            </span>
          </label>
          <label className="block text-sm font-semibold text-slate-800">
            Preview scene
          ...
          {preset && (
            ...
            >
              ...
              {[
                ...
                .filter(([name]) => preset[name].visible !== false)
                .map(([name, label]) => (
                  <label key={name} className="block text-xs text-slate-600">
                    <span className="flex justify-between">
                      ...
                      <span>
                        {Math.round(
                          (opacity[name] ?? preset[name].opacity ?? 1) * 100,
                        )}
                        %
                      </span>
                    </span>
                    <input
                      ...
                      onChange={(event) => {
                        setOpacity((current) => ({
                          ...current,
                          [name]: Number(event.target.value) / 100,
                        }));
                        setRenderUrl("");
                      }}
          ...
          <p className="text-xs text-slate-500">
            Layout preview. Text fits automatically in the exported file.
          </p>
          {!disableBackgroundMusic && (
            <label className="block rounded-xl border border-slate-200 bg-white p-4">
              <span className="mb-2 flex items-center gap-2 text-sm font-semibold text-slate-800">
                <Music size={17} /> Background music (optional)
              </span>
              <input
                type="file"
                accept="audio/*"
                onChange={(event) => setMusic(event.target.files?.[0] || null)}
                className="block w-full text-sm text-slate-600 file:mr-3 file:rounded-lg file:border-0 file:bg-indigo-100 file:px-3 file:py-2 file:font-medium file:text-indigo-700"
              />
              <span className="mt-2 block text-xs text-slate-500">
                Up to 25 MB. Music loops quietly beneath narration in video
                exports.
              </span>
              <span className="mt-2 block text-xs text-slate-600">
                Suggested searches: {musicSearchSuggestions(content).join(", ")}
              </span>
              <a
              ...
              >
                Open YouTube Audio Library
              </a>
              <span className="mt-2 block text-xs text-slate-500">
                Download a suitable track, then upload it here. Paste any
                required attribution below; it will be included in newly
                scheduled posts.
              </span>
              <textarea
                aria-label="Music attribution"
                value={musicCredit}
                onChange={(event) => setMusicCredit(event.target.value)}
                placeholder="Music credit / required attribution (optional)"
          ...
          <div className="flex flex-wrap gap-3">
            <button
              ...
              disabled={busy}
              onClick={handleSave}
              className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 bg-white px-4 py-2.5 text-sm font-semibold text-indigo-700 hover:bg-indigo-50 disabled:opacity-50"
            >
            ...
            >
              <RotateCcw size={17} />{" "}
              {preset ? "Use preset positions" : "Reset"}
            </button>
            <button
              type="button"
              disabled={busy}
              onClick={isImageContent ? handleImageExport : handleRender}
            ...
            {isImageContent && (
              <button
                ...
                className="rounded-xl border border-indigo-200 bg-white px-4 py-2.5 text-sm font-semibold text-indigo-700 disabled:opacity-50"
              >
                Merge as video
              </button>
            )}
          ...
          {renderUrl && (
            <div className="rounded-xl bg-slate-950 p-3">
              ...
              <a
                href={renderUrl}
                download
                className="mt-3 inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-slate-900"
              >
                <Download size={17} /> Download final MP4
              </a>
            </div>
          )}
        </div>
