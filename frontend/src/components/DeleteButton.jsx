...
export default function DeleteButton({ label, description, onDelete, onDeleted }) {
    const [pending, setPending] = useState(false);
    const [error, setError] = useState("");
    const inFlight = useRef(false);

    const handleDelete = async () => {
        if (inFlight.current || !window.confirm(`Delete ${label}?\n\n${description || "This item will be removed from your library."}\nThis cannot be undone.`)) return;
    ...
    return <div className="shrink-0">
        <button type="button" onClick={handleDelete} disabled={pending} aria-label={`Delete ${label}`} className="inline-flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm font-medium text-red-700 hover:bg-red-100 focus-visible:outline-2 focus-visible:outline-red-600 disabled:cursor-wait disabled:opacity-60">
